...
instance rewardNoiseKernel.instIsMarkovKernel [IsMarkovKernel ν] :
...
lemma hasCondDistrib_rewardNoise_history_action {alg : Algorithm (Fin K) ℝ}
  ...
  | succ n =>
      ...
      have h_pair_ae :
          AEMeasurable
            (fun ω ↦ ((IsAlgEnvSeq.hist A R n ω, A (n + 1) ω), R (n + 1) ω)) P :=
        (Measurable.prodMk
          (Measurable.prodMk
            (IsAlgEnvSeq.measurable_hist h.measurable_action h.measurable_feedback n)
      ...
      have h_center_ae :
          AEMeasurable
            (fun p : ((Iic n → Fin K × ℝ) × Fin K) × ℝ ↦
              (p.1, p.2 - (ν p.1.2)[id]))
            (P.map fun ω ↦
              ((IsAlgEnvSeq.hist A R n ω, A (n + 1) ω), R (n + 1) ω)) := by
      ...
      · rw [condDistrib_ae_eq_iff_measure_eq_compProd _ h_noise_ae]
        ...
        calc
          P.map
              (fun ω ↦
                ((IsAlgEnvSeq.hist A R n ω, A (n + 1) ω),
                  rewardNoise A R ν (n + 1) ω))
            = (P.map
              (fun ω ↦
                ((IsAlgEnvSeq.hist A R n ω, A (n + 1) ω), R (n + 1) ω))).map
              (fun p : ((Iic n → Fin K × ℝ) × Fin K) × ℝ ↦
                (p.1, p.2 - (ν p.1.2)[id])) := by
                rw [AEMeasurable.map_map_of_aemeasurable h_center_ae h_pair_ae]
                rfl
          _ = (P.map (fun ω ↦ (IsAlgEnvSeq.hist A R n ω, A (n + 1) ω)) ⊗ₘ
          ...
          _ = P.map (fun ω ↦ (IsAlgEnvSeq.hist A R n ω, A (n + 1) ω)) ⊗ₘ
              (rewardNoiseKernel ν).prodMkLeft (Iic n → Fin K × ℝ) :=
                compProd_map_centerReward_historyAction_eq_compProd_rewardNoiseKernel_prodMkLeft
                  (ν := ν)
                  (μ := P.map fun ω ↦ (IsAlgEnvSeq.hist A R n ω, A (n + 1) ω))
...
lemma rewardNoise_history_action_kernelSubgaussian
    ...
    Kernel.HasSubgaussianMGF id σ2
      ...
      (P.map fun ω ↦ (IsAlgEnvSeq.hist A R (t - 1) ω, A t ω)) :=
...
lemma rewardNoise_map_eq_historyAction_kernel_comp {alg : Algorithm (Fin K) ℝ}
    ...
    P.map (rewardNoise A R ν t) =
      ((rewardNoiseKernel ν).prodMkLeft (Iic (t - 1) → Fin K × ℝ)) ∘ₘ
        P.map (fun ω ↦ (IsAlgEnvSeq.hist A R (t - 1) ω, A t ω)) := by
...
lemma rewardNoise_integrable_exp_mul_history_action {alg : Algorithm (Fin K) ℝ}
  ...
  have hY_ae : AEMeasurable (rewardNoise A R ν t) P :=
    (hasCondDistrib_rewardNoise_history_action (A := A) (R := R) (ν := ν) h ht).aemeasurable_fst
...
lemma rewardNoise_constMul_ae_condExp_exp_le_history_action {alg : Algorithm (Fin K) ℝ}
    ...
    (h_int :
      Integrable
        (fun ω ↦ Real.exp
          (u * (q (IsAlgEnvSeq.hist A R (t - 1) ω, A t ω) *
    ...
    ∀ᵐ ω ∂P,
      P[fun ω' ↦ Real.exp
        ...
        ≤ Real.exp (scalarProjectionVariance σ2
            (q (IsAlgEnvSeq.hist A R (t - 1) ω, A t ω)) * u ^ 2 / 2) := by
  let X : Ω → (Iic (t - 1) → Fin K × ℝ) × Fin K :=
    fun ω ↦ (IsAlgEnvSeq.hist A R (t - 1) ω, A t ω)
  let Y : Ω → ℝ := rewardNoise A R ν t
  let κ : Kernel ((Iic (t - 1) → Fin K × ℝ) × Fin K) ℝ :=
...
lemma rewardNoise_constMul_ae_condExp_exp_sub_realizedVariance_le_one
      ...
      P[fun ω' ↦ Real.exp
          (u * (q (IsAlgEnvSeq.hist A R (t - 1) ω', A t ω') *
            ...
            scalarProjectionVariance σ2
              (q (IsAlgEnvSeq.hist A R (t - 1) ω', A t ω')) * u ^ 2 / 2) |
        (inferInstance : MeasurableSpace ((Iic (t - 1) → Fin K × ℝ) × Fin K)).comap
          (fun ω ↦ (IsAlgEnvSeq.hist A R (t - 1) ω, A t ω))] ω
        ≤ 1 := by
  let X : Ω → (Iic (t - 1) → Fin K × ℝ) × Fin K :=
  ...
  have hf_meas : Measurable f := by
    dsimp [f]
    simp only [scalarProjectionVariance, NNReal.coe_mul, NNReal.coe_mk]
    fun_prop
...
lemma rewardNoise_constMul_hasCondSubgaussianMGF_filtrationAction_of_integrable
    ...
    HasCondSubgaussianMGF
      (IsAlgEnvSeq.filtrationAction h.measurable_action h.measurable_feedback t)
      ((IsAlgEnvSeq.filtrationAction h.measurable_action h.measurable_feedback).le t)
      (fun ω ↦ q (IsAlgEnvSeq.hist A R (t - 1) ω, A t ω) *
        rewardNoise A R ν t ω)
      c P := by
  let ℱ := IsAlgEnvSeq.filtrationAction h.measurable_action h.measurable_feedback
  let mX : MeasurableSpace Ω := ℱ t
...
lemma initialRewardNoise_hasSubgaussianMGF
  ...
  have h_pair :
    ...
    simpa [linUCBAlgorithm, a0] using h.hasLaw_step_zero
  have h_snd :
  ...
  calc
    ∑ t ∈ range n, fullProjectedRewardFeatureNoiseRealizedVariance A σ2 x v t ω
        = ∑ t ∈ range n, (σ2 : ℝ) * (dotProduct v (x (A t ω))) ^ 2 := by
          refine Finset.sum_congr rfl ?_
          intro t _ht
          simp [fullProjectedRewardFeatureNoiseRealizedVariance, scalarProjectionVariance,
            mul_comm]
...
lemma stronglyAdapted_fullProjectedRewardFeatureNoiseRealizedVariance_postActionFiltration
  ...
  have hvar_t :
    ...
    (measurable_of_countable
      (fun a : Fin K ↦ (scalarProjectionVariance σ2 (dotProduct v (x a)) : ℝ))).comp hA_t
  simpa [fullProjectedRewardFeatureNoiseRealizedVariance] using hvar_t.stronglyMeasurable
  ...
  have h_adapted : StronglyAdapted ℱ Y := by
    simpa [ℱ, Y] using
      stronglyAdapted_fullProjectedRewardFeatureNoise_postActionFiltration
        (A := A) (R := R) (ν := ν) (x := x) h.measurable_action h.measurable_feedback v
  have h0 : HasSubgaussianMGF (Y 0) (cY 0) P := by
    simpa [Y, cY, fullProjectedRewardFeatureNoise] using
...
lemma dotProduct_centeredResponseVector_eq_fullProjectedRewardFeatureNoise_sum
  ...
  simp only [dotProduct, WithLp.ofLp_sum, WithLp.ofLp_smul, Finset.sum_apply]
  calc
    ∑ i, v i *
    ...
    _ = ∑ t ∈ range n, dotProduct v (x (A t ω)) * rewardNoise A R ν t ω := by
        refine Finset.sum_congr rfl ?_
        intro t _ht
        simp only [WithLp.ofLp_smul, Pi.smul_apply, smul_eq_mul, dotProduct]
        rw [Finset.sum_mul]
        refine Finset.sum_congr rfl ?_
        intro i _hi
        ring_nf
...
lemma designMatrix_mulVec_linearMeanParameter
  ...
  simp only [designMatrix, meanResponseVector, Matrix.add_mulVec, Matrix.smul_mulVec,
    Matrix.sum_mulVec, Matrix.vecMulVec_mulVec, Matrix.one_mulVec, WithLp.ofLp_sum]
...
lemma designMatrix_mulVec_thetaHat_sub_linearMeanParameter
  ...
  have htheta :
    ...
    simp [thetaHat, V, matrixMulFeature, Matrix.mulVec_mulVec, Matrix.mul_nonsing_inv _ hVdet]
  rw [htheta]
...
lemma centeredResponseDirectionalExponent_sub_priorPenalty_eq_completedSquare
    ...
    centeredResponseDirectionalExponent A R ν reg σ2 x n ω lambda -
      ...
      centeredNoiseQuadraticForm A R ν reg x n ω / (2 * (σ2 : ℝ)) -
        ((σ2 : ℝ) / 2) *
          dotProduct
            (lambda - (σ2 : ℝ)⁻¹ •
              matrixMulFeature (designMatrix A reg x n ω)⁻¹
                (centeredResponseVector A R ν x n ω))
            (Matrix.mulVec (designMatrix A reg x n ω)
              (lambda - (σ2 : ℝ)⁻¹ •
                matrixMulFeature (designMatrix A reg x n ω)⁻¹
  ...
  let y : Feature d := matrixMulFeature V⁻¹ S
  let m : Feature d := σ⁻¹ • y
  have hV : V.PosDef := designMatrix_posDef (A := A) (reg := reg) (x := x)
    (n := n) (ω := ω) hreg_pos
  have hVdet : IsUnit V.det := by
  ...
  have hVy : Matrix.mulVec V y = S := by
    simpa [y] using mulVec_matrixMulFeature_nonsing_inv (M := V) hVdet S
  have hVm : Matrix.mulVec V m = σ⁻¹ • S := by
    simp [m, Matrix.mulVec_smul, hVy]
  have h_lm :
      dotProduct lambda (Matrix.mulVec V m) = σ⁻¹ * dotProduct lambda S := by
    rw [hVm, dotProduct_smul]
    simp [smul_eq_mul]
  have h_yS :
      dotProduct y S = centeredNoiseQuadraticForm A R ν reg x n ω := by
    simp [centeredNoiseQuadraticForm, y, V, S, matrixMulFeature, dotProduct_comm]
  have h_mm :
      dotProduct m (Matrix.mulVec V m) =
        σ⁻¹ * σ⁻¹ * centeredNoiseQuadraticForm A R ν reg x n ω := by
    rw [hVm]
    simp [m, h_yS, dotProduct_smul, smul_dotProduct, smul_eq_mul]
    ring
  have h_shift :
      dotProduct (lambda - m) (Matrix.mulVec V (lambda - m)) =
        dotProduct lambda (Matrix.mulVec V lambda) - 2 * σ⁻¹ * dotProduct lambda S +
          σ⁻¹ * σ⁻¹ * centeredNoiseQuadraticForm A R ν reg x n ω := by
    rw [dotProduct_sub_mulVec_sub_eq hV.isHermitian lambda m, h_lm, h_mm]
    ring
  have h_cancel :
      centeredResponseDirectionalExponent A R ν reg σ2 x n ω lambda -
...
lemma centeredNoiseBiasQuadraticForm_le_sqrt_add_sqrt_sq
  ...
  simpa [centeredNoiseBiasQuadraticForm, centeredNoiseQuadraticForm,
    regularizationBiasQuadraticForm, V, M, u, b] using
    dotProduct_sub_mulVec_sub_le_sqrt_add_sqrt_sq hM u b
...
lemma parameterErrorQuadraticForm_eq_centeredResponseVector_sub_reg_inv_designMatrix
  ...
  have hu : u = matrixMulFeature V⁻¹ z := by
    ext i
    calc
      u i = Matrix.mulVec 1 u i := by simp
      _ = Matrix.mulVec (V⁻¹ * V) u i := by rw [Matrix.nonsing_inv_mul _ hVdet]
      _ = Matrix.mulVec V⁻¹ (Matrix.mulVec V u) i := by rw [Matrix.mulVec_mulVec]
      _ = Matrix.mulVec V⁻¹ z i := by rw [hz]
      _ = matrixMulFeature V⁻¹ z i := rfl
  change dotProduct u (Matrix.mulVec V u) = dotProduct z (Matrix.mulVec V⁻¹ z)
  rw [hz, hu]
  simp [matrixMulFeature, dotProduct_comm]
...
lemma regularizationBias_invDesign_quadraticForm_le
  ...
  calc
    ...
    _ = reg * dotProduct θ θ := by
        rw [reg_smul_one_inv (reg := reg) (d := d) hreg_pos.ne']
        simp [Matrix.smul_mulVec, Matrix.one_mulVec, dotProduct, smul_eq_mul]
  ...
  calc
    |dotProduct u (x a)|
        ≤ √(dotProduct u (Matrix.mulVec V u)) *
          √(dotProduct (x a) (Matrix.mulVec V⁻¹ (x a))) :=
        abs_dotProduct_le_sqrt_mul_sqrt_inv_mulVec hV hVdet u (x a)
