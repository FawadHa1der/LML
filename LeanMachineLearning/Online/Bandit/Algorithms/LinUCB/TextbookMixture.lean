...
omit [IsMarkovKernel ν] in
/-- Assemble the textbook Gaussian-mixture statistic as a `Supermartingale` from integrability and
one-step conditional expectation inputs. -/
...
lemma gaussianDirectionExp_eq_prod (a : ℝ) (lambda : Feature d) :
  ...
  have hsum :
    ...
    simp [dotProduct, pow_two, div_eq_mul_inv, Finset.mul_sum, mul_comm]
...
omit [IsMarkovKernel ν] [IsProbabilityMeasure P] in
/-- Product formula for coordinate-wise weighted Gaussian exponential integrals. -/
...
lemma measurable_centeredResponseDirectionalExponent_prod
  ...
  have h_lambda : ∀ j : Fin d, Measurable fun p : Ω × Feature d ↦ p.2 j := by
    intro j
    exact (PiLp.continuous_apply (p := 2) (β := fun _ : Fin d ↦ ℝ) j).measurable.comp
      measurable_snd
...
omit [IsMarkovKernel ν] [IsProbabilityMeasure P] in
/-- Center used when completing the square in the textbook Gaussian integral. -/
noncomputable def completedSquareGaussianCenter
    ...
    Feature d :=
  (σ2 : ℝ)⁻¹ •
    matrixMulFeature (designMatrix A reg x n ω)⁻¹ (centeredResponseVector A R ν x n ω)
...
omit [IsMarkovKernel ν] [IsProbabilityMeasure P] in
/-- Build the concrete Gaussian-prior input from the design-eigenvector integral identity. -/
...
/-- Build the concrete Gaussian-prior input from the fixed-direction supermartingale integral. -/
...
/-- First positive mixture-bound failure time before the finite horizon, defaulting to zero. -/
...
omit [IsMarkovKernel ν] [IsProbabilityMeasure P] in
/-- Optional-stopping expectation bound for a bounded stopping time. -/
lemma integral_stoppedValue_le_initial_of_supermartingale
  ...
  have hle : (fun _ : Ω ↦ (0 : ℕ∞)) ≤ τ := fun ω ↦ zero_le (τ ω)
  have hmono :=
